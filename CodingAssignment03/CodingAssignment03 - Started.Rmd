...
```{r q1}
remove3 <- insurance[-c(13,22,47),]
removequalitative <- remove3[,c('Charges','Age','BMI','Children')]
summary(removequalitative)
...
```{r}
cor(removequalitative)
corrplot(cor(removequalitative),
type = "lower",
order = "hclust",
tl.col = "black",
tl.srt = 45,
...
```{r q3}
model <- lm(Charges~ Age + BMI + Female + Children + Smoker +WinterSprings + WinterPark + Oviedo, data = remove3)
summary(model)
